...
def process_single_file(file_path):
    """
    Process a single markdown file and return chunks ready for embedding
    """
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            if len(chunk.strip()) > 10:  # Skip very small chunks
                ...
                chunk_id = f"{os.path.basename(file_path)}_{i:03d}"
                chunk_info.append({
                    "id": chunk_id,
                    "title": title,
                    "path": file_path,
        ...
        return texts_to_embed, chunk_info
...
def ingest_documents():
    ...
    for md_file in md_files:
        # Process single file
        texts_to_embed, chunk_info = process_single_file(md_file)
        ...
        try:
            # Generate embeddings using Cohere in smaller batches if needed
