...
uvicorn qdrant_rag_api:app --host 127.0.0.1 --port 8000 &
sleep 3
...
echo ""
echo "4) Testing query endpoint with book question"
curl -X POST http://127.0.0.1:8000/query \
-H "Content-Type: application/json" \
-d '{"query": "What is Physical AI?"}'

echo -e "\n\n5) Testing query endpoint with another question"
curl -X POST http://127.0.0.1:8000/query \
-H "Content-Type: application/json" \
-d '{"query": "Explain machine learning"}'

echo -e "\n\n6) Testing query endpoint with a third question"
curl -X POST http://127.0.0.1:8000/query \
-H "Content-Type: application/json" \
